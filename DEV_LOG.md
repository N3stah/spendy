...
What I did: Created spendy.py with a config block, three functions with docstrings, and an if __name__ guard. The program asks for a description and amount then prints them back formatted.

What confused me: I wasn't sure why we needed if __name__ == "__main__" since the program seemed to work without it. I also didn't fully understand why get_description() needed to be its own function when it's just one line.

How I figured it out: Kimi explained that if __name__ == "__main__" protects the code from running automatically if the file ever gets imported elsewhere. For the functions, I understood it better when I thought about Step 04 and Step 05 — having everything in functions now means I won't have to rewrite messy code later.
...
What confused me: I didn't understand why we used a dictionary instead of just saving the description and amount separately.

How I figured it out: Kimi explained that dictionaries make it easy to add more fields like category and date later without rewriting everything.
...
How I figured it out: I learned it prevents the csv module from adding extra blank lines between rows on some computers.
...
What confused me: I wasn't sure if I should check for zero and negative numbers in the same place or separately.

How I figured it out: I kept it simple — one check at a time in the same while loop, so each problem gets its own clear error message.
...
How I figured it out: I put it before the loop so it only shows once when you open the program, and the help command handles reprinting it later.
...
What happened: The old expenses.csv had no header row so 
csv.DictReader could not find the "description" column and crashed.
How I fixed it: Deleted the old file so spendy.py could create a 
fresh one with the correct header. Also added a header check in 
load_expenses() so it never crashes on a bad file again.
